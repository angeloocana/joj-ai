...
function getNMovesToWin(pieces: J.IPiece[], isBlack: boolean): J.IPiece[][][] {
    ...
    pieces.forEach(piece =>
        piece.movesToWin = this.getDistance(piece, isBlack));
...
function getPiecesFurtherAway(pieces: J.IPiece[], isBlack: boolean): number {
    ...
    let lastMoveToWin = -1;
    const nMovesToWin = this.getNMovesToWin(pieces, isBlack);
    for (let i = 0; i < nMovesToWin.length; i++) {
        const nMoves = this.getLastMoveToWin(nMovesToWin[i]);
...
function getBestMove2(game: J.IGame, moves: J.IMove[], isBlack: boolean): J.IMove {
    ...
    for (let i = 0; i < moves.length; i++) {
        ...
        pieces = J.Board.getPiecesWhereCanIGo(isBlack, boardAfterMove, pieces);
        const furtherPiece =
...
function getMoveNearWinPosition(allPieces: J.IPiece[],
    ...
    const choosenMove = this.getBestMove(game, (positions.length > 0 ? positions : moves), isBlack);
    return choosenMove;
}
...
function addBestMovesAfterMove(piece: J.IPiece,
    fromY: number, fromBeforeMove: J.IPosition,
    toBeforeMove, game: J.IGame, isBlack: boolean,
    bestMoves, winPositions) {

    const gameCopy = game.getNewCopy();
    gameCopy.move(fromBeforeMove, toBeforeMove);

    const whereCanIGoAfterMove = gameCopy.board.getPositionsWhereCanIGo(piece, isBlack);
    if (piece.whereCanIGo.orderedPositions.length < whereCanIGoAfterMove.orderedPositions.length) {
        ...
        for (let p = 0; p < higherPositions.length; p++) {
            ...
            if (to) {
                to.lastMove = { from: fromBeforeMove, to: toBeforeMove };
                const toY = J.Position.getY0Start(to.y, isBlack);
                bestMoves = this.addBestMove(bestMoves, (toY - fromY), fromBeforeMove, toBeforeMove, 1, isBlack);
                winPositions = this.addWinPositions(piece, to, winPositions, 1, isBlack);
            }
        }
    }
...
function getBestMove(game: J.IGame): J.IMove {
    ...
    this.getNMovesToWin(allPieces, isBlack);
                ...
                if (tos)
                    for (let i = 0; i < tos.length; i++) {
                        ...
                        if (to) {
                            ...
                            bestMoves = this.addBestMove(bestMoves, (toY - fromY), piece, to, 0, isBlack);
                            bestMoves = this.addBestMovesAfterMove(piece, fromY, piece, to, game, isBlack, bestMoves, winPositions);

                            winPositions = this.addWinPositions(piece, to, winPositions, 0, isBlack);
                        }
                    }

                if (piece.whereCanIGo.orderedPositions.length < 8) {
                    const otherPieces = PieceHelper.getOtherPieces(allPieces, piece);
                    for (let n = 0; n < otherPieces.length; n++) {
                        ...
                        for (let o = 0; o < otherPiece.whereCanIGo.length; o++) {
                            ...
                            bestMoves = this.addBestMovesAfterMove(piece, fromY, otherPiece, otherPieceMove, game, isBlack, bestMoves, winPositions);
    ...
    const bestLevel = this.getBestLevel(bestMoves);
    const higherBestMoves = bestMoves[bestLevel][bestMoves[bestLevel].length - 1];

    const move = this.getMoveNearWinPosition(allPieces, higherBestMoves,
