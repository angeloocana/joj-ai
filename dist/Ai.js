...
var Ai = function () {
    ...
    _createClass(Ai, [{
        ...
        value: function addBestMoviment(bestMoviments, positionsAHead, startPosition, nextPosition, nivel, isBlack) {
            ...
            var positionsAHeadOnMoviment = _jojCore.BoardHelper.getY0Start7End(nextPosition.y, isBlack) - _jojCore.BoardHelper.getY0Start7End(startPosition.y, isBlack);
            ...
            if (winPosition && winPosition.length > 0 && winPosition[0].nextPosition) {
                for (var i = 0; i < winPosition.length; i++) {
                    if (winPosition[i].nextPosition) {
                        ...
                        if (_jojCore.BoardHelper.getY0Start7End(move.startPosition.y, isBlack) < 7) return move;
                    }
                }
            }
        }
        ...
        value: function getDistance(piece, isBlack) {
            var winPositions = [];
            for (var x = 0; x < 8; x++) {
                var y = _jojCore.BoardHelper.getY7Start0End(piece.y, isBlack);
                ...
                if (moviments > 1) {
                    for (var i = 0; i < piece.whereCanIGo.positions.length; i++) {
                        ...
                        var nextPositionY = _jojCore.BoardHelper.getY7Start0End(nextPosition.y, isBlack);
        ...
        value: function isGameEnding(allPieces, isBlack) {
            for (var i = 0; i < allPieces.length; i++) {
                ...
                if (_jojCore.BoardHelper.getY0Start7End(piece.y, isBlack) < 4) return false;
            }
            return true;
        }
    }, {
        ...
        value: function addWinPositions(startPosition, nextPosition, winPositions, nivel, isBlack) {
            if (!nextPosition) {
                if (_jojCore.BoardHelper.getY0Start7End(startPosition.y, isBlack) === 7) {
                ...
                if (_jojCore.BoardHelper.getY0Start7End(nextPosition.y, isBlack) === 7) {
            ...
            if (piece.whereCanIGo.orderedPositions.length < whereCanIGoAfterMove.orderedPositions.length) {
                ...
                for (var p = 0; p < higherPositions.length; p++) {
                    ...
                    if (nextPosition) {
                        ...
                        var nextPositionY = _jojCore.BoardHelper.getY0Start7End(nextPosition.y, isBlack);
        ...
        value: function getBestMove(game) {
            ...
            var orderedPieces = _jojCore.PieceHelper.getPiecesOrdered(allPieces, isBlack);
            for (var l = 0; l < orderedPieces.length; l++) {
                if (orderedPieces[l]) {
                    ...
                    for (var m = 0; m < pieces.length; m++) {
                        ...
                        var startPositionY = _jojCore.BoardHelper.getY0Start7End(piece.y, isBlack);
                        ...
                        if (piece.whereCanIGo.orderedPositions.length < 8) {
                            var otherPieces = _jojCore.PieceHelper.getOtherPieces(allPieces, piece);
